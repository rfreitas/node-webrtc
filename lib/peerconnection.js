...
function RTCPeerConnection() {
  ...
  pc.onaudioframe = function(id, bitsPerSample, sampleRate, numberOfChannels, numberOfFrames, buffer) {
      var audioFrameEvent = new RTCAudioFrameEvent('audioframe',
          { id,
      ...
      self.dispatchEvent(audioFrameEvent);
      if (self._onaudioframe) self._onaudioframe(audioFrameEvent);
  };

  pc.onvideoframe = function(id, width, height, yPlane, uPlane, vPlane) {
    var videoFrameEvent = new RTCVideoFrameEvent('videoframe',
        {id, width, height, yPlane, uPlane, vPlane});
    self.dispatchEvent(videoFrameEvent);
    if (self._onvideoframe) self._onvideoframe(videoFrameEvent);
  };

  pc.onencodedvideoframe = function(id, width, height, encodedFrame, frameType, timestamp) {
      var videoFrameEvent = new RTCVideoFrameEvent('videoframe',
          {id, width, height, encodedFrame, frameType, timestamp});
      self.dispatchEvent(videoFrameEvent);
      if (self._onencodedvideoframe) self._onencodedvideoframe(videoFrameEvent);
  };
...
RTCPeerConnection.prototype.getStats = function getStats() {
  if (typeof arguments[0] === 'function') {
    ...
    var onFailure = arguments[1];
    this._pc.legacyGetStats().then(function(internalRTCStatsResponse) {
